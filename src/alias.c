/* **********************************************************
	alias.c Aliasing module for Dikumud						
	does everything to do with aliases.						

	Written by Dave Oldcorn. Tested on DikuST.
        Contact Dave at dro%umist-ee-fs2@man.ac.uk

Applies the aliases on the command entered - so if (for example)
a god uses 'force mithos kill guard' this is ONLY processed through
the god's aliases not through mithos' too. (e.g. if the god had an
alias kg = kill guard, force mithos kg would send "Dave has forced
you to 'kill guard'."). If you are SWITCHED you can still use your
aliases.

Syntax: alias <text> <replacement text> - sets an alias
        alias <text> - removes that alias
        alias - lists your current aliases

Aliasing ALIAS is a very bad idea. If you do this you must contact an
immortal to remove any aliases you have.


BUGS - none known
PROBLEMS
-	Has a large memory overhead cos of all the CREATE calls
	involved to store an alias. Then again the most it could
	be is about 1K per character so that should be OK. But lots of
	aliases and removing of aliases will fragment memory.
-	In certain situations it may get a bit confused if you allow
	abbreviation of the word ALIAS - it will pre-substitute into
	alias-remove commands and get very confused. Or if someone manages
	to alias alias although I have tried to prevent this.


 To install add:
	do_alias   command to interpreter.c
		possibly do_alia to prevent probs, see above (optional)

	char *alias_result;		(definitions, game_loop, comm.c)

	alias_result = substitute_aliases(point->character, comm);
	command_interpreter(point->character, alias_result);
			(instead of the command_interpreter call in
				game_loop, comm.c. Remember f-prototypes)

	delete_aliases(ch);		(to free_char, db.c)

	ALIAS in HELP screen, a HELP ALIAS entry (that text at the top)

	add directory alias\ (or alias/) in the diku library directory.

	if on a MSDOS system change ALIAS_FILENAME below
...
void do_alias(struct char_data *ch, char *argument, int cmd)
{
	...
	if (!*alias_name) {
		...
		if (!(alias_entry = alias_data->first)) {
			send_to_char("Aliases? What aliases?!\n\r",ch);
			return;
		}
...
struct alias_data_type *load_aliases(struct char_data *alias_ch)
{
	...
	{
		sprintf(buf, "%s no alias file", alias_ch->player.name);
		log(buf);
		return(alias_data);
	...
	fclose(alias_file);
	sprintf(buf, "%s loaded %d aliases", alias_ch->player.name, num);
	log(buf);
	return(alias_data);
}
			...
			char *alias_name, char *alias_text)
{
	...
	if (alias_data->number >= ((GET_LEVEL(alias_ch) < IMM_5) ?
