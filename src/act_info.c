...
void show_char_to_char(struct char_data *i, struct char_data *ch, int mode)
{
	...
	if ((ch != i) || GET_LEVEL(ch)>31){
	    found = FALSE;
	    send_to_char("\n\rYou attempt to peek at the inventory:\n\r", ch);
	    for(tmp_obj = i->carrying; tmp_obj;
		tmp_obj = tmp_obj->next_content) {
		if (CAN_SEE_OBJ(ch, tmp_obj) && number(0,20) < GET_TT(ch)) {
...
void do_score(struct char_data *ch, char *argument, int cmd)
{
    ...
    send_to_char(buf,ch);
    sprintf(buf,"You have attained these levels: [Ma: %2d  Cl: %2d  Th: %2d  Wa: %2d]\n\r",
	    GET_MM(ch), GET_CC(ch), GET_TT(ch), GET_WW(ch));
    send_to_char(buf,ch);
/*    if (GET_LEVEL(ch)<31){
...
void do_help(struct char_data *ch, char *argument, int cmd)
{
	...
	for (;;)
	{
	    ...
	    if (!(chk = strncmp(argument, help_index[mid].keyword,strlen(argument))))
	    {
...
void do_who(struct char_data *ch, char *argument, int cmd)
{
    ...
    char fullbuf[MAX_STRING_LENGTH];
    char buf[256];
    char tmp[256];
    char temp[256];
    ...
    /* New parser -- kind of ugly, but not bad for not knowing C 
       New Who system written by Alander Imp. of Rivers of Mud
       cie-2.uoregon.edu 9000 (aka Russ Taylor)
   */
    count = 1;
    while (count <= 3)
    {
    ...
    sprintf(buf,"[Ma Cl Th Wa]----Players---------\n\r");
    strcat(fullbuf, buf);
	...
	if ((!d->connected) &&
	    (CAN_SEE(ch, d->character) || (GET_LEVEL(ch) >= 34)) &&
	     ((low_limit <= GET_CLASS(d->character))) &&
	     ((GET_CLASS(d->character) <= high_limit))&&
            ((check_class == -1)  ||
             ((check_class == CLASS_MAGIC_USER) &&
             ...
             ))
	{
	  if (IS_NPC(d->character))
	    sprintf(temp,"MOB");
	  else if (GET_LEVEL(d->character) == 35)
	    sprintf(temp,"[    GOD    ] ");
	  else if (GET_LEVEL(d->character) == 34)
	    sprintf(temp,"[  SUPREME  ] ");
	  else if (GET_LEVEL(d->character) == 33)
	    sprintf(temp,"[   DEITY   ] ");
	  else if (GET_LEVEL(d->character) == 32)
	    sprintf(temp,"[  IMMORTAL ] ");
	  else if (GET_LEVEL(d->character) == 31)
	    sprintf(temp,"[    HERO   ] ");
	  else 
	    sprintf(temp,"[%2d %2d %2d %2d] ",GET_MM(d->character),
		    GET_CC(d->character) ,GET_TT(d->character),
		    GET_WW(d->character));
	  if (!strcmp(temp,"MOB"))
	    continue;
...
/*	  send_to_char(buf, ch);  */
  ...
  if (current >= maximum)
	maximum = current;
  sprintf(buf,"\n\rThe most players we've had is %d.\n\r", maximum);
  strcat(fullbuf, buf);
  sprintf(buf,"The current number of players is %d\n\r", current);
  strcat(fullbuf, buf);
  page_string(ch->desc, fullbuf, 1);
}
...
void do_sockets(struct char_data *ch, char *argument, int cmd)
{
	...
	if ( (!CAN_SEE(ch, d->character)) || (GET_LEVEL(ch) < GET_LEVEL(d->character)))
	    continue;
	...
	{
	case CON_PLAYING:
	    sprintf(buf,"CON_PLAYING\n\r"); break;
	case CON_GET_NAME:
	    sprintf(buf,"CON_GET_NAME\n\r"); break;
	case CON_GET_OLD_PASSWORD:
	    sprintf(buf,"CON_GET_OLD_PASSWORD\n\r"); break;
	case CON_CONFIRM_NEW_NAME:
	    sprintf(buf,"CON_CONFIRM_NEW_NAME\n\r"); break;
	case CON_GET_NEW_PASSWORD:
	    sprintf(buf,"CON_GET_NEW_PASSWORD\n\r"); break;
	case CON_CONFIRM_NEW_PASSWORD:
	    sprintf(buf,"CON_CONFIRM_NEW_PASSWORD\n\r"); break;
	case CON_GET_NEW_SEX:
	    sprintf(buf,"CON_GET_NEW_SEX\n\r"); break;
	case CON_GET_NEW_CLASS:
	    sprintf(buf,"CON_GET_NEW_CLASS\n\r"); break;
	case CON_READ_MOTD:
	    sprintf(buf,"CON_READ_MOTD\n\r"); break;
	case CON_SELECT_MENU:
	    sprintf(buf,"CON_SELECT_MENU\n\r"); break;
	case CON_RESET_PASSWORD:
	    sprintf(buf,"CON_RESET_PASSWORD\n\r"); break;
	case CON_CONFIRM_RESET_PASSWORD:
	    sprintf(buf,"CON_CONFIRM_RESET_PASSWORD\n\r"); break;
	case CON_EXDSCR:
	    sprintf(buf,"CON_EXDSCR\n\r"); break;
	default:
	    sprintf(buf,"***UNKNOWN***\n\r"); break;
    }
    strcat(fullbuf, buf);
