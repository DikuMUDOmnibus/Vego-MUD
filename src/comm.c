...
void game_loop( int control )
{
	...
	extern int bzero( );	/* For FD_ZERO ick! */
	...
	for (point = descriptor_list; point; point = next_point) 
	{
		...
		else
		{
			...
			GET_MOVE(point->character) );
		      ...
		      sprintf( buf, "< %d > ",world[point->character->in_room].number);
...
int new_descriptor(int s)
{
	...
	*newd->host = '\0';
    ...
    {
	sprintf( log_buf, "Sock.sinaddr:  %d.%d.%d.%d", 
		sock.sin_addr.s_net,
    ...
    {
	if ( !str_cmp( newd->host, tmp->name) != NULL)
	{
	    write_to_descriptor( desc,
...
void send_to_level(char *messg, int level, CHAR_DATA *ch)
{
    ...
    if (messg)
    {
      strcpy(buf, messg);
      for (i = descriptor_list; i; i = i->next)
             if(!i->connected && GET_LEVEL(i->character) >= MAX(IMM_LVL,level))
