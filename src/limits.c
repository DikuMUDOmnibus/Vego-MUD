...
/* move gain pr. game hour */
{
	...
	gain += 4*(GET_CON(ch) + GET_DEX(ch)) / divisor;
    }
...
void advance_level(struct char_data *ch)
{
    ...
    add_practices = GET_CLASS(ch);
    GET_CL(ch)[(int) GET_CLASS(ch)]++;
    for (i=1,GET_LEVEL(ch)=GET_CL(ch)[(int) GET_CLASS(ch)];i<5;i++) 
      if (GET_LEVEL(ch)<GET_CL(ch)[i]) 
      { 
      GET_CLASS(ch)=i;
      GET_LEVEL(ch)=GET_CL(ch)[i];
      }

    switch(add_practices)
    {
    ...
    add_hp                       = (add_hp*(1000-ch->points.max_hit))/1000;
    add_hp			 = MAX( 1, add_hp);
    add_mana			 = MAX( 0, add_mana);
    add_moves			 = MIN( 0, add_moves);
    add_practices                = 3 + (GET_CLASS(ch)==add_practices);
    ...
    sprintf( buf,
	"Your gain is: %d/%d hp, %d/%d m, %d/%d mv %d/%d gain points.\n\r",
...
void gain_condition(struct char_data *ch,int condition,int value)
{
    ...
    GET_COND(ch,condition) = MIN(60,GET_COND(ch,condition));
