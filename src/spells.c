...
int use_mana( struct char_data *ch, int sn )
{
    ...
    divisor = MAX(2+GET_CC(ch)-spell_info[sn].min_level_cleric,
		  2+GET_MM(ch)-spell_info[sn].min_level_magic);
...
void say_spell( struct char_data *ch, int si )
{
  ...
  { "tect", "infra" },
	...
	if(temp_char != ch) {
	    if (GET_CLASS(ch) == GET_CLASS(temp_char)||GET_LEVEL(ch)>31)
...
*/
  ...
  if ( ((spl > 0 && spl <= 44) || spl >= 53) && spell_info[spl].spell_pointer )
  {
    ...
    break;
      ...
      if (GET_LEVEL(ch) < 32) {
	if ((spell_info[spl].min_level_magic > GET_MM(ch))
	    && spell_info[spl].min_level_cleric > GET_CC(ch)) {
    ...
    send_to_char("Ok.\n\r",ch);
    ((*spell_info[spl].spell_pointer) (MAX(GET_CC(ch),GET_MM(ch)), ch, argument, SPELL_TYPE_SPELL, tar_char, tar_obj));
...
/*  SPELLO( 9,12,POSITION_STANDING,15, 32, 40,
    ...
    SPELLO(75,12,POSITION_FIGHTING, 25, 31, 21, TAR_CHAR_ROOM,
	   cast_demonfire);
