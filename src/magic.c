  ...
  struct char_data *victim, struct obj_data *obj)
{
  ...
  if (obj) {
    ...
    if (obj->obj_flags.type_flag == ITEM_WEAPON && obj->obj_flags.value[2] > 1)
     obj->obj_flags.value[2]--; 
     act("$p glows red, but then fades back into its original color.", FALSE, ch, obj, 0, TO_CHAR);
       }
       else {
    ...
    af.type      = SPELL_CURSE;
    af.duration  = 5*level;
    af.modifier  = (GET_LEVEL(ch)>20?-4:-2);
    af.location  = APPLY_HITROLL;
  ...
  struct char_data *victim, struct obj_data *obj)
{
  ...
  if ((GET_ITEM_TYPE(obj) == ITEM_WEAPON) &&
    ...
    if (IS_GOOD(ch)&&GET_LEVEL(ch)<34) {
      SET_BIT(obj->obj_flags.extra_flags, ITEM_ANTI_EVIL);
      act("$p glows blue.",FALSE,ch,obj,0,TO_CHAR);
    } else if (IS_EVIL(ch)&&GET_LEVEL(ch)<34) {
...
void spell_gate(byte level, struct char_data *ch,
		struct char_data *victim, struct obj_data *obj)
{
if (ch==victim || IS_NPC(victim)) return;
#ifdef CREATE_MODE 
 return;
#endif
...
act("$n folds out of existence.. You see a faint picture of $N for a moment.",
    FALSE, ch, 0, victim, TO_ROOM);
char_from_room(ch);
char_to_room(ch,victim->in_room);
act("$n unfolds into existence, kneeling humbly before $N.",FALSE,ch,0,victim,TO_ROOM);
act("Your sense of time and space distorts momentarily.",FALSE,ch,0,0,TO_CHAR);
do_look(ch,"",15);
}
