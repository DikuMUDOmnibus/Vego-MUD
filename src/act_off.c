...
bool is_first_level(struct char_data *ch, struct char_data *victim)
{
    ...
    if ((!IS_NPC(victim))&&(GET_LEVEL(ch)<5))
    {
	send_to_char(
	    "You may not attack a player until you are level 5.\n\r",ch);
	return TRUE;
    }
...
void do_kill(struct char_data *ch, char *argument, int cmd)
{
	...
	if (ch == victim)
	  ...
	  act("You grab hold of $M and crush $M just like a grape! Ah! The blood!",
		FALSE, ch, 0, victim, TO_CHAR);
	  act("$N grabs hold of your head and pops it off like a dandelion!", FALSE, victim, 0, ch, TO_CHAR);
	  act("$n grabs hold of $N's head and squeezes until it explodes.", FALSE, ch, 0, victim, TO_NOTVICT);
	  raw_kill(victim);
	}
    }
}
...
void do_rescue(struct char_data *ch, char *argument, int cmd)
{
    ...
    if (0) /*(GET_CLASS(ch) != CLASS_WARRIOR) && GET_LEVEL(ch)<33) */
