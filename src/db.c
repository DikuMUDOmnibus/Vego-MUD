...
int top_of_online = 0;                /* ref to top of online space*/
...
int top_of_mobt_online = 0;
int top_of_objt = 0;                  /* top of object index table       */
int top_of_objt_online = 0;
...
void boot_db(void)
{
    ...
    top_of_mobt_online=top_of_mobt+1;
    generate_indices(obj_f, &top_of_objt, obj_index);
    top_of_objt_online=top_of_objt+1;
    renum_zone_table();
...
void renum_mobs()
{
    ...
    for (;;)
        ...
        if (fgets(buf, 81, mob_f))
	  {
            if (*buf == '#')
	      {
		if (atoi(&buf[1])==mob_index[i].virtual)
		  {
		  ...
		  {
		    mob_index[real_mobile(atoi(&buf[1]))].pos=ftell(mob_f);
		  }	
	      }
	    else if (*buf == '$')    /* EOF */
	      break;
        ...
        if (fgets(buf, 81, obj_f))
          {
            if (*buf == '#')
              {
                if (atoi(&buf[1])==obj_index[i].virtual)
                  {
                  ...
                  {
                    mob_index[real_object(atoi(&buf[1]))].pos=ftell(obj_f);
                  }
              }
            else if (*buf == '$')    /* EOF */
              break;
...
struct char_data *read_mobile(int nr, int type)
{
    ...
    fscanf(mob_f, " %s \n", buf);
    for(i=0;(i<MAX_ATTACKS) && (*buf=='K') ; i++)
      {
	fscanf(mob_f," %d %d %d ",&tmp, &tmp2, &tmp3);
	mob->specials.attack[i].dice=tmp;
	mob->specials.attack[i].size=tmp2;
	mob->specials.attack[i].bonus=tmp3;

	fscanf(mob_f, " %d \n", &tmp);
	mob->specials.attack[i].type=tmp;
	fscanf(mob_f, " %s \n",buf);
      }
...
void free_char( CHAR_DATA *ch )
{
    ...
    if (ch->amsg.head) free(ch->amsg.head);
    if (ch->amsg.body) free(ch->amsg.body);
    ch->amsg.head=NULL;
    ch->amsg.body=NULL;
...
int i;
    ...
    for(i=0;i<MAX_ATTACKS;i++) {
      ch->specials.attack[i].dice=0;
      ch->specials.attack[i].size=0;
      ch->specials.attack[i].bonus=0;
      ch->specials.attack[i].type=0;
    }
...
int real_room(int virtual)
{
	...
	if (bot >= top)
	{
	  for(bot=top_of_world+1;bot<=top_of_online;bot++)
	    if ((world+bot)->number==virtual)
{
  log("Testing online");
  fprintf(stderr,"Bot %d\n",bot);
  return(bot);
}
...
int real_mobile(int virtual)
{
	...
	if ((mob_index + mid)->virtual == virtual)
	    return(mid);
	if (bot >= top) {
	    for (bot=top_of_mobt;bot<top_of_mobt_online;bot++)
	      if ((mob_index+bot)->virtual == virtual) return(bot);
	    return(-1);
	  }
	if ((mob_index + mid)->virtual > virtual)
	    top = mid - 1;
...
int real_object(int virtual)
{
	...
	if ((obj_index + mid)->virtual == virtual)
	    return(mid);
	if (bot >= top) {
	  for (bot=top_of_objt;bot<top_of_objt_online;bot++)
	    if ((obj_index+bot)->virtual == virtual) return(bot);
	    return(-1);
	}
	if ((obj_index + mid)->virtual > virtual)
	    top = mid - 1;
...
int create_online(int loc)
{
int room_nr=0,tmp;

room_nr=real_room(loc);
log("Creating room..");
top_of_online++;
room_nr=top_of_online;
world[room_nr].number=loc;
fprintf(stderr,"CREATED: ROOM NUM: %d",loc);
world[room_nr].name=str_dup("--");
world[room_nr].description=str_dup("-----");
world[room_nr].zone=0;
world[room_nr].room_flags=0;
...
void save_online_rooms(struct char_data *ch)
{
  ...
  orm=rm=-4;
  log("Saving world..");
  tmp = fopen("../lib/tinyworld.wld","w");
  for (i=0;i<top_of_world;)
    {
      if (world[i].number<=orm) i++;
      rm=world[i].number;
      for (j=top_of_world;j<=top_of_online;j++)
      if (world[j].number > orm)
	rm=MIN(rm,world[j].number);
      orm=rm;
      rm=real_room(orm);
      fprintf(tmp,"#%d\n",world[rm].number);
      fprintf(tmp,"%s~\n",world[rm].name);
      fprintf(tmp,"%s~\n",world[rm].description);
      fprintf(tmp,"%d %d %d\n",world[rm].zone,world[rm].room_flags,
	      world[rm].sector_type);
      for (j=0;j<6;j++) {
	if (world[rm].dir_option[j])
	  {
	    fprintf(tmp,"D%d\n%s~\n%s~\n",j,
		    world[rm].dir_option[j]->general_description,
		    world[rm].dir_option[j]->keyword);
	    fprintf(tmp,"%d %d %d\n",world[rm].dir_option[j]->exit_info,
		    world[rm].dir_option[j]->key,
		    (world[rm].dir_option[j]->to_room>0?world[world[rm].dir_option[j]->to_room].number:-1));
	  }
      }
      for (new_descr = world[rm].ex_description;new_descr;new_descr=new_descr->next)
	{
	  fprintf(tmp,"E\n%s~\n%s~\n",new_descr->keyword,new_descr->description);
	}
      fprintf(tmp,"S\n");
    }
  fprintf(tmp,"#32000\n$~\n\n");
...
void save_online_mob(struct char_data *ch,struct char_data *k)
{
char buf[MAX_STRING_LENGTH];
int i;
FILE *wld,*tmp;
tmp=fopen("../lib/tinyworld.mob.conv","w+");
wld=fopen("../lib/tinyworld.mob","r+");

while(fgets(buf,MAX_STRING_LENGTH,wld)!=NULL)
{
  if (buf[0]=='#')
    {
      if (atoi(&buf[1])>=mob_index[k->nr].virtual)
	{
	  log("Saving");
	  ...
	  fprintf(tmp,"%d %d %d\n",k->specials.act,k->specials.affected_by,k->specials.alignment);
	  for (i=0;i<MAX_ATTACKS&&k->specials.attack[i].type;i++)
	    fprintf(tmp,"K\n%d %d %d %d\n",k->specials.attack[i].dice,
		    k->specials.attack[i].size,k->specials.attack[i].bonus,
		    k->specials.attack[i].type);
	  fprintf(tmp,"S\n%d %d %d 0d0+%d %dd%d+%d\n",GET_LEVEL(k),20 - k->points.hitroll,
		  (k->points.armor)/10,k->points.max_hit,k->specials.damnodice,
		  k->specials.damsizedice,k->points.damroll);
	  fprintf(tmp,"%d %d\n%d %d %d\n",k->points.gold,GET_EXP(k),k->specials.position,k->specials.default_pos,k->player.sex);
	  if (atoi(&buf[1])==mob_index[k->nr].virtual) {
	    do {
	      fgets(buf,MAX_STRING_LENGTH,wld);
	    } while(buf[0]!='#');
	  }
	  do {
	    fprintf(tmp,"%s",buf);
...
rewind(tmp);
rewind(wld);
while(fgets(buf,MAX_STRING_LENGTH,tmp)!=NULL)
     fprintf(wld,"%s",buf);
fflush(wld);
fclose(tmp);
fclose(wld);
...
void save_online_obj( struct obj_data *k)
{
char buf[MAX_STRING_LENGTH];
int i;
FILE *wld,*tmp;
...
if ((tmp=fopen("../lib/tinyworld.obj.conv","w+"))==NULL) log("GOT LOG TMP");
if ((wld=fopen("../lib/tinyworld.obj","r+"))==NULL) log("GOT WLD THING");
rewind(wld);
rewind(tmp);
...
while(fgets(buf,MAX_STRING_LENGTH,wld)!=NULL)
{
  if (buf[0]=='#')
    {
      if (atoi(&buf[1]) >= obj_index[k->item_number].virtual)
        {
          log("Saving");
          fprintf(tmp,"#%d\n",obj_index[k->item_number].virtual);
	  log("printed once");
	  fflush(stderr);
	  fprintf(tmp,"%s~\n",k->name);
	  log("Gasp");
	  ...
	  log("TOPP");
	  fflush(stderr);
	  fprintf(tmp,"%s~\n",k->action_description);
	  log("Twice");
	  fflush(stderr);
	  fprintf(tmp,"%d %d %d\n",k->obj_flags.type_flag,
		  k->obj_flags.extra_flags,k->obj_flags.wear_flags);
	  fprintf(stderr,"WWW\n");
	  fflush(stderr);
	  fprintf(tmp,"%d %d %d %d\n",k->obj_flags.value[0],
		  k->obj_flags.value[1],k->obj_flags.value[2],
		  k->obj_flags.value[3]);
	  fprintf(tmp,"%d %d %d\n",k->obj_flags.weight,k->obj_flags.cost,
		  k->obj_flags.cost_per_day);
	  log("extra descr");
	  fflush(stderr);
	  ...
	  log("Affects");
	  for (i=0; (i<MAX_OBJ_AFFECT) && (k->affected[i].location);i++) {
	    fprintf(tmp,"A\n%d %d\n",k->affected[i].location,k->affected[i].modifier);
	  }
	  log("looping");
	  fflush(stderr);
          if (atoi(&buf[1])==obj_index[k->item_number].virtual) {
