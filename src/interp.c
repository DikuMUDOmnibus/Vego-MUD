...
struct command_info cmd_info[] =
{
    ...
    { "look",       do_look,        POSITION_RESTING,   0,  15 },
...
/*  { "order",      do_order,       POSITION_RESTING,   0,  9 },    */
    ...
    { "shout",      do_shout,       POSITION_RESTING,   3,  9 },
    { "gossip",     do_shout,       POSITION_RESTING,   3, 10 },
    { "auction",    do_shout,       POSITION_RESTING,   3, 11 },
/*  { "tell",       do_tell,        POSITION_RESTING,   0,  9 },    */
...
/*  { "get",        do_get,         POSITION_RESTING,   0,  9 },    */
    ...
    { "read",       do_read,        POSITION_RESTING,   0,  63 },
    { "recite",     do_recite,      POSITION_RESTING,   0,  9 },
    { "remove",     do_remove,      POSITION_RESTING,   0,  66 },
...
/*  { "cast",       do_cast,        POSITION_SITTING,   0,  9 },    */
    { "color",      do_color,       POSITION_SLEEPING,  0,  9 },
...
/*  { "look",       do_look,        POSITION_RESTING,   0,  9 },    */
    ...
    { "write",      do_write,       POSITION_STANDING,  1,  149 },
    ...
    { "system",     do_system,      POSITION_DEAD,      35, 9 },
    { "?",	    do_godline,     POSITION_DEAD,	35, 13},
    { "god",	    do_godline,     POSITION_DEAD,	35, 13},
    { "string",     do_string,      POSITION_DEAD,	35, 9 },
    { "mallin",     do_demote,      POSITION_DEAD,      35, 9 },
    { "jail",       do_jail,        POSITION_DEAD,	35, 9 },
    { "sget",       do_sget,        POSITION_DEAD,	35, 9 },
    { "pceq",       do_pceq,        POSITION_DEAD,	35, 9 },
    { "pcinv",      do_pcinv,       POSITION_DEAD,      35, 9 },

    { ">",	    do_supline,     POSITION_DEAD,	34, 12},
    { "supreme",    do_supline,     POSITION_DEAD,      34, 12},
    ...
    { "setobj",     do_setobj,      POSITION_DEAD,	34, 9 },
    { "teleport",   do_teleport,    POSITION_DEAD,	34, 9 },
    { "setmob",     do_setmob,      POSITION_DEAD,	34, 9 },
    { "load",       do_load,	    POSITION_DEAD, 	34, 9 },

    { "<",	    do_deiline,     POSITION_DEAD,	33, 11},
    { "deity",	    do_deiline,     POSITION_DEAD,	33, 11},
    { "disconnect", do_disconnect,  POSITION_DEAD,	33, 9 },
    { "wizlock",    do_wizlock,     POSITION_DEAD,      33, 9 },
    { "snoop",      do_snoop,	    POSITION_DEAD,	33, 9 },
    { "setroom",    do_setroom,	    POSITION_DEAD,	33, 9 },
    { "pardon",     do_pardon,      POSITION_DEAD,      33, 9 },
    { "restore",    do_restore,     POSITION_DEAD,      33, 9 },
    { "trans",      do_trans,       POSITION_DEAD,      33, 9 },
     
    { "sockets",    do_sockets,	    POSITION_DEAD,	32, 9 },
    { "purge",      do_purge,	    POSITION_DEAD,	32, 9 },
    { "noshout",    do_noshout,	    POSITION_DEAD,	32, 9 },
    { "increment",  do_increment,   POSITION_DEAD,      32, 9 },
    { "force",      do_force,	    POSITION_DEAD,	32, 9 },
    { "at",         do_at,          POSITION_DEAD,      32, 9 },
    { "echo",       do_echo,        POSITION_DEAD,      32, 9 },
    { "goto",       do_goto,        POSITION_DEAD,      32, 9 },
    { "quiet",      do_quiet,       POSITION_DEAD,      32, 9 },
    { "stat",       do_stat,        POSITION_DEAD,      32, 9 },
    { "switch",     do_switch,      POSITION_DEAD,      32, 9 },
    { "invis",      do_wizinvis,    POSITION_DEAD,      32, 9 },

    { ":",	    do_immline,    POSITION_DEAD,	31, 10},
    { "immortal",   do_immline,    POSITION_DEAD,	31, 10},
...
void command_interpreter( struct char_data *ch, char *pcomm )
{
     ...
     * Log players.
     */
    if ( 1 || ((!IS_NPC(ch)) && ((GET_LEVEL(ch) > 31) || 
       (IS_SET(ch->specials.act, PLR_LOG)))))
    {
	sprintf( log_buf, "Log %s: %s",
        (IS_MOB(ch) ? ch->player.short_descr : GET_NAME(ch)), pcomm );
	log( log_buf );
    }
...
void do_wizhelp(struct char_data *ch, char *argument, int cmd_arg)
{
    ...
    send_to_char(
	"The following privileged commands are available:\n\r\n\r", ch);
