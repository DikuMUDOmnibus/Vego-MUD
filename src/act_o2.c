...
void do_drink(struct char_data *ch, char *argument, int cmd)
{
	...
	if((GET_COND(ch,FULL)>50)&&(GET_COND(ch,THIRST)>50)) /* Stomach full */
	  {
    ...
    if((GET_COND(ch,FULL)>50)&&(GET_COND(ch,THIRST)>0)) /* Stomach full */
      {
	act("Your stomach can't contain anymore!",FALSE,ch,0,0,TO_CHAR);
	return;
      }
...
void wear(struct char_data *ch, struct obj_data *obj_object, int keyword)
{
	...
	case 0: {  /* LIGHT SOURCE */
		...
		equip_char(ch,obj_object, WEAR_LIGHT);
                sprintf (buffer,
                "You wear the %s as a light source.\n\r",
	...
	case 2: {
		...
		&& (ch->equipment[WEAR_NECK_2])) {
		    ...
		    if (ch->equipment[WEAR_NECK_1]) {
			 sprintf (buffer,
                         "You wear the %s around your neck.\n\r",
                          fname(obj_object->name));
			send_to_char (buffer, ch);
                        obj_from_char(obj_object);
	...
	case 3: {
		...
		if (ch->equipment[WEAR_BODY]) {
		    ...
		    perform_wear(ch,obj_object,keyword);
                    sprintf (buffer,
     		    "You wear the %s for protection of your torso.\n\r",
     		     fname(obj_object->name));
		    send_to_char (buffer, ch);
		    obj_from_char(obj_object);
	...
	case 4: {
		...
		if (ch->equipment[WEAR_HEAD]) {
		    ...
		    perform_wear(ch,obj_object,keyword);
		    sprintf (buffer,
    		    "You wear the %s protecting your head.\n\r",
     		    fname(obj_object->name));
		    send_to_char (buffer, ch);
		    obj_from_char(obj_object);
	...
	case 5: {
		...
		if (ch->equipment[WEAR_LEGS]) {
		    ...
		    perform_wear(ch,obj_object,keyword);
		    sprintf (buffer,
     		    "You wear the %s for leg protection.\n\r",
     		     fname(obj_object->name));
		    send_to_char (buffer, ch);
		    obj_from_char(obj_object);
	...
	case 6: {
		...
		if (ch->equipment[WEAR_FEET]) {
		    ...
		    perform_wear(ch,obj_object,keyword); 
		    sprintf (buffer,
     		    "You wear the %s to cover your feet.\n\r",
    		     fname(obj_object->name));
		    send_to_char (buffer, ch);
		    obj_from_char(obj_object);
	...
	case 7: {
		...
		if (ch->equipment[WEAR_HANDS]) {
		    ...
		    perform_wear(ch,obj_object,keyword);
                    sprintf (buffer,
   		    "You wear the %s to protect your hands in combat.\n\r",
   		     fname(obj_object->name));
		    send_to_char (buffer, ch);
		    obj_from_char(obj_object);
	...
	case 8: {
		...
		if (ch->equipment[WEAR_ARMS]) {
		    ...
		    perform_wear(ch,obj_object,keyword); 
		    sprintf (buffer,
     		    "You wear the %s as a form of arm protection.\n\r",
      		     fname(obj_object->name));
		    send_to_char (buffer, ch);
		    obj_from_char(obj_object);
	...
	case 9: {
		...
		if (ch->equipment[WEAR_ABOUT]) {
		    ...
		    perform_wear(ch,obj_object,keyword); 
		   sprintf (buffer,
     		   "You wear the %s as a cover for your whole body.\n\r",
       		    fname(obj_object->name));
		    send_to_char (buffer, ch);
		    obj_from_char(obj_object);
	...
	case 10: {
		...
		if (ch->equipment[WEAR_WAISTE]) {
		    ...
		    perform_wear(ch,obj_object,keyword);		    
  		    sprintf (buffer,
 	    	    "You wear the %s about your waist.\n\r",
      		     fname(obj_object->name));
		    send_to_char (buffer, ch);
		    obj_from_char(obj_object);
	...
	case 12:
		...
		if (ch->equipment[WIELD]) {
			...
			perform_wear(ch,obj_object,keyword);
		        sprintf (buffer,
     		        "You wield the %s and prepare for battle.\n\r",
      			 fname(obj_object->name));
			send_to_char (buffer, ch);
			obj_from_char(obj_object);
	...
	case 13:
		...
		if (ch->equipment[HOLD]) {
		    ...
		    perform_wear(ch,obj_object,keyword);
		    sprintf (buffer,
     		    "You hold the %s for a security blanket.\n\r",
      		     fname(obj_object->name));
		    send_to_char (buffer, ch);
		    obj_from_char(obj_object);
...
/*        if (ch->specials.fighting) {
	  send_to_char("With that nasty thing fighting you?!?\n\r",ch);
	  return;
	}
